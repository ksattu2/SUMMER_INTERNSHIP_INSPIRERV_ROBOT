...
#define I2C_STAR1_ERR_MASK                                                               \
    (I2C_STAR1_PECERR | I2C_STAR1_OVR | I2C_STAR1_AF | I2C_STAR1_ARLO | I2C_STAR1_BERR)

#define I2C_INT_EN_MASK (I2C_CTLR2_ITBUFEN | I2C_CTLR2_ITEVTEN | I2C_CTLR2_ITERREN)

typedef enum { I2C_RESULT_OK = 0, I2C_TIMEOUT_NOT_BUSY, I2C_TIMEOUT_NO_ACK } i2c_result_e;
...
i2c_result_e i2c_write(uint16_t devAddr, uint16_t regAddr,
    i2c_regAddr_bytes_e regAddrBytes, uint8_t * data, uint8_t sz);
i2c_result_e i2c_read(uint16_t devAddr, uint16_t regAddr,
    i2c_regAddr_bytes_e regAddrBytes, uint8_t * data, uint8_t sz);
...
i2c_result_e i2c_ping(uint8_t addr, uint8_t * found) {
    ...
    while ((!i2c_chk_evt(I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) && (timeout--))
        ;
    if (timeout == -1)
        reply = 0;                 // if time out - no ack from the  device
...
/**
 * @brief AT24C256 (EEPROM) forbids sequential write across multiple pages (one page = 64
 * bytes) This is a wrapper function for the original i2c_write() in ch32v003_i2c.h
 */
i2c_result_e i2c_write_pages(uint16_t devAddr, uint16_t regAddr,
    i2c_regAddr_bytes_e regAddrBytes, uint8_t * data, uint16_t sz);

/**
 * @brief Original function i2c_read() takes sz parameter as uint8_t, which is not enough
 * for EEPROM with 32KB capacity This is a wrapper function for the original i2c_read() in
 * ch32v003_i2c.h
 */
i2c_result_e i2c_read_pages(uint16_t devAddr, uint16_t regAddr,
    i2c_regAddr_bytes_e regAddrBytes, uint8_t * data, uint16_t sz);
